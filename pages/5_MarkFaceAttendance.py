...
from Utils.Paths import ENCODING_FILE_NAME
...
try:
    with open(ENCODING_FILE_NAME, "rb") as file:
        encodedData = pickle.load(file)
    ids, names, knownEncodedEmbeddings = encodedData
except FileNotFoundError:
    st.warning(f"⚠️ Encoding file '{ENCODING_FILE_NAME}' not found. Please add students first.")
    st.stop()
